...
export function AppDrawer({ onClose }: AppDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div
        className="relative w-full max-w-[390px] bg-white rounded-t-[28px] z-10"
        style={{
          paddingBottom: 80, // leave space for the nav bar below
          maxHeight: '82vh',
          overflowY: 'auto',
      ...
      >
        ...
        <div className="px-5 pb-3 pt-1 flex items-center justify-between">
          <span className="text-[18px] font-extrabold text-gray-800">Všechny sekce</span>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-500 text-[18px]"
          >×</button>
        </div>
                ...
                {section.items.map(item => {
                  ...
                  return (
                    <Link
                      ...
                      style={{
                        background: active ? 'var(--color-primary-light)' : '#F4F6FA',
