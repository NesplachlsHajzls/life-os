...
function AddDebtSheet({ prefillPerson, onSave, onClose }: { prefillPerson?: string; onSave: (e: Omit<Debt, 'id'>) => void; onClose: () => void }) {
  ...
  const fieldCls = 'w-full bg-gray-50 border border-gray-200 rounded-[12px] px-3.5 py-2.5 text-[14px] text-gray-800 placeholder-gray-400 outline-none focus:border-green-400'
                ...
                <div key={personKey} className="bg-white rounded-[16px] shadow-card overflow-hidden">
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0 mr-2">
                      <div className="text-[15px] font-bold text-green-600">+{fmt(total)} Kč</div>
                    ...
                    <div className="border-t border-gray-100">
                      {entries
                        ...
                        .map((entry, idx) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <div className="text-[13px] font-semibold text-gray-800">{entry.note || '—'}</div>
                              <div className="text-[11px] text-gray-400 mt-0.5">
                                {formatDate(entry.date_from)}
                                {entry.date_to ? ` → ${formatDate(entry.date_to)}` : ''}
                              </div>
                            </div>
                            <span className="text-[13px] font-bold text-green-600 flex-shrink-0">+{fmt(entry.amount)} Kč</span>
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-4 py-2.5 text-[12px] font-semibold text-gray-400 hover:text-green-600 hover:bg-gray-50 transition-colors"
                      >
