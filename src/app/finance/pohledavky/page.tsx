...
function AddDebtSheet({ prefillPerson, onSave, onClose }: { prefillPerson?: string; onSave: (e: Omit<Debt, 'id'>) => void; onClose: () => void }) {
  ...
  const fieldCls = 'w-full bg-[var(--bg)] border border-[var(--border)] rounded-[12px] px-3.5 py-2.5 text-[14px] text-[var(--text-primary)] placeholder-gray-400 outline-none focus:border-green-400'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[390px] bg-[var(--surface)] rounded-t-[24px] p-5 pb-8 max-h-[92vh] overflow-y-auto">
        ...
        <h2 className="text-[16px] font-bold mb-4">💚 Nová pohledávka</h2>
        <div className="flex flex-col gap-4">
          <div>
            <label className={labelCls}>Osoba / firma</label>
            <input className={fieldCls} value={person} onChange={e => setPerson(e.target.value)} placeholder="Jirka, Pepík, Firma..." />
          </div>
          <div>
          ...
          <div>
            ...
            <input className={fieldCls} value={note} onChange={e => setNote(e.target.value)} placeholder="půjčka, záloha, oběd..." />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              onClick={() => { if (valid) { onSave({ person: person.trim(), amount: +amount, note, date_from: dateFrom, date_to: dateTo || undefined, type: 'receivable' }); onClose() } }}
              disabled={!valid}
              className="flex-1 py-3 rounded-[14px] text-[14px] font-bold text-white bg-green-500 disabled:opacity-40"
            >Přidat</button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[390px] bg-[var(--surface)] rounded-t-[24px] p-5 pb-8 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-4">
          <div><label className={labelCls}>Osoba / firma</label><input className={fieldCls} value={person} onChange={e => setPerson(e.target.value)} /></div>
          <div><label className={labelCls}>Částka (Kč)</label><input type="number" className={fieldCls} value={amount} onChange={e => setAmount(e.target.value)} /></div>
          <div><label className={labelCls}>Za co</label><input className={fieldCls} value={note} onChange={e => setNote(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Od kdy</label><input type="date" className={fieldCls} value={dateFrom} onChange={e => setDateFrom(e.target.value)} /></div>
            <div><label className={labelCls}>Do kdy</label><input type="date" className={fieldCls} value={dateTo} onChange={e => setDateTo(e.target.value)} /></div>
          </div>
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 py-3 rounded-[14px] border border-[var(--border)] text-[14px] font-semibold text-[var(--text-secondary)]">Zrušit</button>
            <button
...
export default function PohledavkyPage() {
  ...
  // Only receivables on this page
  const myReceivables = debts.filter(d => d.type === 'receivable')

  const grouped = myReceivables.reduce<Record<string, Debt[]>>((acc, d) => {
  ...
  const totalOwed = myReceivables.reduce((s, d) => s + d.amount, 0)
  ...
  async function handleQuickAdd() {
    ...
    if (!parsed) { setQuickError('Zkus: "Jirka 500 oběd" nebo "Martin 3000 záloha"'); return }
    setQuickError('')
    await addDebt({ ...parsed, date_from: todayStr(), type: 'receivable' })
    setQuickInput('')
  }
  ...
  return (
    <>
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {/* Hero card */}
        <div className="rounded-[20px] p-5 text-white" style={{ background: 'linear-gradient(135deg, #16a34a, #15803d)' }}>
          <div className="text-[11px] font-semibold opacity-70 uppercase tracking-wide mb-1">Dluží mi celkem</div>
          {loading
        ...
        <div className="bg-[var(--surface)] rounded-[16px] shadow-card px-4 py-3.5">
          <div className="text-[10px] font-bold text-green-500 uppercase tracking-wide mb-2">➕ Přidat pohledávku</div>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Jirka 500 oběd / Martin 3000 záloha"
            />
            <button onClick={handleQuickAdd} disabled={!quickInput.trim()} className="px-4 py-2.5 rounded-[12px] bg-green-500 text-white text-[13px] font-bold disabled:opacity-40">+</button>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-[var(--text-tertiary)] text-[13px]">
            Nikdo ti nic nedluží 🎉<br /><span className="text-[12px]">Přidej pohledávku výše</span>
          </div>
        ) : (
                ...
                <div key={personKey} className="bg-[var(--surface)] rounded-[16px] shadow-card overflow-hidden">
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0 mr-2">
                      <div className="text-[15px] font-bold text-green-600">+{h(total)}</div>
                    ...
                    <div className="border-t border-[var(--border)]">
                      {entries
                        ...
                        .map((entry, idx) => (
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              <div className="text-[13px] font-semibold text-[var(--text-primary)]">{entry.note || '—'}</div>
                              <div className="text-[11px] text-[var(--text-tertiary)] mt-0.5">
                                {formatDate(entry.date_from)}
                                {entry.date_to ? ` → ${formatDate(entry.date_to)}` : ''}
                              </div>
                            </div>
                            <span className="text-[13px] font-bold text-green-600 flex-shrink-0">+{h(entry.amount)}</span>
                      ...
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-4 py-2.5 text-[12px] font-semibold text-[var(--text-tertiary)] hover:text-green-600 hover:bg-[var(--bg)] transition-colors"
                      >
