...
export default function NoteDetailPage() {
  ...
  // Editable state (synced from note)
  ...
  useEffect(() => {
    if (!noteId) return
    let cancelled = false
    Promise.all([fetchNote(noteId), fetchSubNotes(noteId)])
      .then(([n, subs]) => {
        if (cancelled) return
        ...
        setSubNotes(subs)
        setLoading(false)
        // Load parent if nested note
        if (n.parent_id) {
          fetchNote(n.parent_id).then(p => { if (!cancelled) setParent(p) })
        }
        // Load client name if meeting
  ...
  // ── Autosave ─────────────────────────────────────────────────
  const scheduleSave = useCallback((newTitle: string, newContent: string, newIcon: string) => {
    if (!note) return
    ...
    saveTimer.current = setTimeout(async () => {
      setSaveState('saving')
      try {
        await updateNote(note.id, { title: newTitle || 'Nová poznámka', content: newContent, icon: newIcon })
        setNote(prev => prev ? { ...prev, title: newTitle || 'Nová poznámka', content: newContent, icon: newIcon } : prev)
        setSaveState('saved')
  ...
  function handleTitleChange(val: string) {
    setTitle(val)
    scheduleSave(val, content, icon)
  }

  function handleContentChange(val: string) {
    setContent(val)
    scheduleSave(title, val, icon)
  }

  function handleIconChange(newIcon: string) {
    setIcon(newIcon)
    setShowIconPicker(false)
    scheduleSave(title, content, newIcon)
  }
  ...
  const handleNewSubNote = useCallback(async () => {
    ...
    try {
      const sub = await insertNote({
        ...
        is_meeting: false, meeting_date: null, icon: '📝',
      })
  ...
  async function handleDelete() {
    ...
    // Schůzka → zpět ke klientovi; vnořená poznámka → zpět na rodiče; jinak → poznámky
  ...
  // Schůzka → zpět ke klientovi; vnořená poznámka → zpět na rodiče; jinak → /poznamky
    ...
    <div className="min-h-screen bg-[#F4F6FA] flex flex-col">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-1.5 flex-1 min-w-0 text-[12px] text-gray-400">
          {isMeeting ? (
            // Schůzka: Práce / Název klienta / Název schůzky
            <>
              <Link href="/prace" className="hover:text-gray-600 transition-colors flex-shrink-0">Práce</Link>
              {clientName && (
                <>
                  <span className="flex-shrink-0">/</span>
                  <Link href={`/prace/${note.client_id}`} className="hover:text-gray-600 transition-colors truncate max-w-[140px]">
                    {clientName}
                  </Link>
                </>
              )}
            ...
            // Poznámka: Poznámky / [rodič] / Název
            <>
              <Link href="/poznamky" className="hover:text-gray-600 transition-colors flex-shrink-0">Poznámky</Link>
              {parent && (
                <>
                  <span className="flex-shrink-0">/</span>
                  <Link href={`/poznamky/${parent.id}`} className="hover:text-gray-600 transition-colors truncate max-w-[120px]">
                    {parent.title}
                  </Link>
                </>
              )}
        ...
        {/* Meeting badge */}
        {note.is_meeting && (
          <span className="flex-shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600">
            🤝 Schůzka
          </span>
        )}
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={e => {
                ...
                setNote(prev => prev ? { ...prev, meeting_date: val } : prev)
                if (note) updateNote(note.id, { meeting_date: val || null })
              }}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="relative flex-shrink-0">
            <button onClick={() => setShowIconPicker(v => !v)}
              className="text-[40px] hover:bg-gray-100 rounded-[12px] p-1 transition-colors leading-none"
              title="Změnit ikonu">
              {icon}
            </button>
            {showIconPicker && (
              ...
              {subNotes.map(sub => (
                <Link key={sub.id} href={`/poznamky/${sub.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {sub.content && (
                      <div className="text-[11px] text-gray-400 truncate">{sub.content.slice(0, 60)}</div>
                    )}
                  </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center"
          ...
          <div className="bg-white rounded-[20px] p-6 mx-4 w-full max-w-[360px] shadow-2xl"
            onClick={e => e.stopPropagation()}>
            <div className="text-[18px] font-extrabold text-gray-900 mb-2">Smazat poznámku?</div>
            <p className="text-[13px] text-gray-500 mb-5">
              Smaže se i všechny podstránky. Tuto akci nelze vrátit.
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={handleDelete}
                className="flex-1 py-3 rounded-[14px] bg-red-500 text-white text-[14px] font-bold">
                Smazat
              </button>
            </div>
          </div>
